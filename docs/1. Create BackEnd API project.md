...
6. A continuación, crearemos un nuevo Entity Framework DbContext. Cree una nueva clase `ConferenciaDbContext` en la carpeta` Models` usando el siguiente código:
   ```csharp
    ...
    namespace BackEnd.Models
    {
        ...
        {
            public ApplicationDbContext(DbContextOptions<ConferenciaDbContext> options)
                : base(options)
            {
...
```console
dotnet aspnet-codegenerator controller -api -name PonentesController -m Ponente -dc BackEnd.Modelos.ApplicationDbContext -outDir Controllers
```
